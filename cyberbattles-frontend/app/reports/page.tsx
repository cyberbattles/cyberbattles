...
import { CheckCircle, XCircle, TrendingUp, Award, Target, AlertCircle } from 'lucide-react';
      ...
      const [challenges, setChallenges] = useState<ChallengeItem[]>([
        {
          ...
          attempts: 2,
          segments: [
            { type: 'text', content: 'This challenge demonstrates a web server that is exploitable by players via an SQL attack. The login page for this web server has an incorrect setup for logging users in.' },
            { type: 'code', content: "sql = \"SELECT note FROM users WHERE id = '%s' AND passwd = '%s'\" % (username, passwd)" },
            { type: 'text', content: 'This SQL query setup allows an always-true payload. Entering an SQL attack query that is always true would return rows the attacker should not see and log the user into the server as admin.' },
            { type: 'heading', content: 'Attack Side' },
            { type: 'text', content: 'To access the flag attackers can use this always-true payload to access the server and solve the challenge.' },
            { type: 'code', content: "... OR '1'='1'" },
            { type: 'heading', content: 'Defence Side' },
            { type: 'text', content: 'In order to defend against the vulnerability the script should treat the values as non-SQL not as SQL commands. ' },
            { type: 'code', content: "sql = \"SELECT note FROM users WHERE id = ? AND passwd = ?\"" },
            { type: 'text', content: 'The database driver does not splice username and password into the SQL string. Instead it sends the SQL template and the parameter values separately to the database engine. The DB treats those parameters purely as data, never as SQL syntax.' },
            { type: 'text', content: 'To understand more about this challenge and how SQL attacks can occur, CyberBattles recommends this reference.' },
          ],
    ...
    return (
        <div className="min-h-screen pt-16 px-6 pb-12 text-white">
            ...
            <div className="mb-12">
              ...
              <p className="text-gray-400 text-lg">Review challenges from your last game and improve your skills</p>
            </div>
            {/* Completed Challenges */}
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-6">
                <h2 className="text-2xl font-bold text-green-400">Completed Challenges</h2>
              </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-white font-bold">{challenge.attempts}</span>
                      </div>
                      <div className="pt-2">
            ...
            <div className="mb-16">
              <div className="flex items-center gap-3 mb-6">
                <h2 className="text-2xl font-bold text-red-400">Challenges to Master</h2>
              </div>
            ...
            {incompleteChallenges.length > 0 && (
              <div className="border-t border-gray-700 pt-12">
                <div className="flex items-center gap-3 mb-8">
                  <h2 className="text-2xl font-bold text-white">Challenge Solutions & Strategies</h2>
                </div>
                  ...
                  {incompleteChallenges.map((challenge) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-2xl font-bold text-white mb-2">Challenge Solution</h3>
                          <span className={`text-xs px-3 py-1 rounded-full ${getDifficultyColor(challenge.difficulty)}`}>
                            {challenge.difficulty}
                          </span>
                        </div>
